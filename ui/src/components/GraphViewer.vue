...
<script>
    ...
    function constructBezierCurvePath(d) {
        const {
            p1x,
            p1y
    ...
    function getBezierCurveMidPoint(d) {
        const {
            p1x,
            p1y
        } = getBezierCurveQuadraticPointP1(d);
    ...
    function getBezierCurveQuadraticPointP1(d) {
    ...
    function pan(dx, dy) {
        ...
        document.getElementById("map-matrix").setAttributeNS(null, "transform", "matrix(" +  transMatrix.join(' ') + ")");
    }
    ...
    function initialize() {
        ...
        simulation = d3.forceSimulation()
            ...
            .force("center", d3.forceCenter(graphWindow.width/2, graphWindow.height/2));
    }
    ...
    export default {
        ...
        methods: {
            ...
            tripleComponentValue(tc) {
                if (tc.type === "uri") {
                    ...
                    return this.getIriMapPropValue(tc.value, 'iriShortPfx') + ":" + IRIComponents[1];
                } else {
                    return tc.value;
                }
            },
            triplesToGraph(triples) {
                ...
                triples.forEach(function(triple) {
                    const subjId = that.tripleComponentValue(triple.s);
                    const predId = that.tripleComponentValue(triple.p);
                    const objId = that.tripleComponentValue(triple.o);

                    let subjNode = filterNodesById(graph.nodes, subjId)[0];
                    let objNode = filterNodesById(graph.nodes, objId)[0];

                    if (subjNode == null) {
                        subjNode = { id: subjId, label: subjId.split(':')[0], weight: 1, type: "node", tripleType: "subject", color: that.getIriMapPropValue(triple.s.value, "color"), textColor: that.getIriMapPropValue(triple.s.value, "textColor"), radius: that.getIriMapPropValue(triple.s.value, "radius") };
                    ...
                    if (objNode == null) {
                        ...
                        if (triple.o.type === 'uri') {
                            ...
                            objNode.color = that.getIriMapPropValue(triple.o.value, "color");
                            objNode.textColor = that.getIriMapPropValue(triple.o.value, "textColor");
                            objNode.radius = that.getIriMapPropValue(triple.o.value, "radius");
                    ...
                    const predNode = { id: predId, label: predId.split(':')[1].camelToSnake(), weight: 1, type: "pred", tripleType: "predicate", color: that.getIriMapPropValue(triple.p.value, "color"), textColor: that.getIriMapPropValue(triple.p.value, "textColor") };
        ...
        watch: {
            value(value) {
                if (value !== undefined) {
                    update(this.triplesToGraph(value.results.bindings));
                }
            }
        }
    }
...
<style scoped>
    div#graphViewerComponent >>> .node {
        stroke: #abcdef;
        /*fill:#ddd;*/
        stroke-width: 1.5px;
    }

    div#graphViewerComponent >>> .link {
        fill: none;
        stroke: #999;
        stroke-opacity: .6;
    }

    div#graphViewerComponent >>> marker {
        stroke: #999;
        fill:rgba(124,240,10,0);
    }

    div#graphViewerComponent >>> .node-text {
        font: 12px sans-serif;
        /*fill:black;*/
    }

    div#graphViewerComponent >>> .link-text {
        font: 11px sans-serif;
        fill:grey;
    }

    div#graphViewerComponent >>> svg {
        border:1px solid black;
    }

    div#graphViewerComponent >>> span#text-length-holder-pred {
        position: absolute;
        top: -20px;
        left: 10px;
        visibility: hidden;
    ...
    div#graphViewerComponent >>> span#text-length-holder-node {
        position: absolute;
        top: -20px;
        left: 10px;
        visibility: hidden;
    ...
    div#graphViewerComponent >>> .compass {
        fill:   #fff;
        stroke: #000;
        stroke-width:   1.5;
    }

    div#graphViewerComponent >>> .button {
        fill: #225EA8;
        stroke: #0C2C84;
        stroke-miterlimit: 6;
    ...
    div#graphViewerComponent >>> .button:hover {
        stroke-width: 2;
    }

    div#graphViewerComponent >>>  .plus-minus {
        fill:   #fff;
        pointer-events: none;
    }
</style>